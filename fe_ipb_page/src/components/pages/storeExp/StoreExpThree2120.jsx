...
//             if (dupSkuList.includes(item.id) && item.addData > -1 && item.addData <= 3) {
//               return(
//                 <tr key={item.id}>
//                   <td></td>
//                     <td>
    ...
    axios(url_be, {
      ...
      .then((res) => {
        ...
        const addData = res.data.map((item) => ({
          ...item,
          addData: subtractDates(todayDate, item.exp),
        })).filter((item) => item.addData > -1 && item.addData <= 3); // 필터링 조건 추가
...
export default StoreExpThree2120;
